"""
StatIQ — FastAPI Router: Football
Endpoints: scrape/stats, matches, leagues, train, predict, predictions
...
async def _run_football_stats_bg(session_id: str, league_key: str):
    ...
    try:
        ...
        # Save team stats to Supabase so ML model can use real data
        ...
        await queue.put({
            "type":    "done",
            "message": f"{league_key}: {saved} partidos, {stats_saved} equipos con stats",
...
async def train_football_model(background_tasks: BackgroundTasks):
    ...
    async def _train():
        ...
        await queue.put({"type": "done", "message": "Entrenamiento futbol finalizado", "result": result})
...
async def predict_football_match(match_id: str):
    supabase  = get_supabase()
    ...
    # Fetch latest team stats if available
    ...
    supabase.table("predictions").insert({
        ...
        "confidence":        pred["p_home_win"] if pred["predicted_winner"] == "home"
                             else (pred["p_draw"] if pred["predicted_winner"] == "draw"
                             else pred["p_away_win"]),
